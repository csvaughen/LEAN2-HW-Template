...
-- we did these examples in the assignment LEAN1
...
-- Exercise: replace the words "sorry" with the correct Lean justification.
/-
   Note the next example is from the "Mechanics of Proof" textbook, section 1.1
   It is given in both example 1.1.3 and 1.2.3 and is interesting as an application
   of Brahmagupta's identity.
-/
...
example {a b c d e f : ℤ} (h1 : a * d = b * c) (h2 : c * f = d * e) :
    ...
    calc
     d * (a * f - b * e)
      = d*a*f - d*b*e := by ring
      _ = f*(a*d) - b*(d*e) := by ring
      _ = f*(b*c) - b*(c*f) := by rw [h1,h2]
      _ = 0 := by ring
    done
